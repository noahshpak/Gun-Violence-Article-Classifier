...
def rule_based_classifier(data):
	...
	for label, text in data :
		...
		if "was shot" in text : prediction = '1'
		if "shot to death" in text: prediction = '1'
		if "gang violence" in text : prediction = '1'
		if "gunshot wound" in text : prediction = '1'
...
def get_top_features(X, y, dv):
	...
	feature_names = dv.get_feature_names() #list data type

	print clf.coef_
	print len(clf.coef_[0])
	print "Shooting Weight: " + str(clf.coef_[0][feature_names.index("shooting")])
	print "Gun Weight: " + str(clf.coef_[0][feature_names.index("gun")])
	print "Violence Weight: " + str(clf.coef_[0][feature_names.index("violence")])
	print "Shot Weight: " + str(clf.coef_[0][feature_names.index("shot")])
